...
class DCA:
    ...
    def sub_grad_collection(self,len_F,Z,tao):
        ...
        for f in range(len_F):
            ...
            s_range_min_to_kstarp_sublist = itertools.combinations(s_range_min_to_max, self.k_star+2-smin)
    ...
    def k_sum(self, Z, f, start, end, tao):
        ...
        return grad_f
    def k_sum(self,Z,f,sublist,tao):
        z_s_values = np.array([Z[f].queue[s][1] for s in sublist])
        # 计算累减值并乘以 tao[f]，然后累加到 grad_f 中
        grad_f = -np.sum(z_s_values,axis=1) * tao[f]
        return grad_f
    def grad_h(self,Z,tao):
        grad_h=np.zeros(shape=(self.len_M))
        for f in range(self.len_F):
            for s in range(self.k_star_plus):
                l_wave,z_s=Z[f].queue[s]
                grad_h-=z_s*tao[f]
        return grad_h
