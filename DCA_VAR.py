...
class DCA:
    ...
    def forward(self, a_r, b_r, c_r, g, k, o):
        ...
        while (i < self.i_max):
            ...
            #获取h的次梯度
            h_sub_grad_list = self.sub_grad_collection(self.len_F, L,tao)
            # if (len(h_sub_grad) == 1):
            #     y_cur = h_sub_grad[0]
            g_x = cp.multiply(cp.inv_pos(x), a_multi_g)
            h_x = cp.Variable()
            h_x.value=0
            ...
            y_best=h_sub_grad_list[0]
            min_value=100000000000000000000000000000000000000000000000000000000
            if(len(h_sub_grad_list)>1):     #当次梯度集大于1时，进行计算
                for h_sub_grad in h_sub_grad_list:      #枚举求解
                    y_cur.value=h_sub_grad
                    cur_value=self.get_g_y_conj_value(g_x,y_cur,x,g_x_constraint)
                    if(cur_value<=min_value):
                        y_best=y_cur.value
            y_process_value.append(y_best)
            y_cur.value=y_best
            #y_best即为当前的y_t
            #接下来求x_t+1
            P4_C=[]
            ...
            P4_C.extend(P4_C2)
            P4_C.extend(P4_C3)
            P4_Obj=cp.Minimize(cp.multiply(cp.inv_pos(x), a_multi_g)-cp.sum(x*y_cur)+cp.sum(v*tao))
            P4=cp.Problem(P4_Obj,P4_C)
            P4.solve()
            x_best=x.value
            x_process_value.append(x_best)
            g_process_value.append(g_x.value)
            h_process_value.append(h_x.value)



        print(np.array(g_process_value)-np.array(h_process_value))
        print(time.time())
        return x_process_value

    def sub_grad_collection(self, len_F, L, tao):
        ...
        for f in range(len_F):
            ...
            # 子集的列表
            left_grad_f = self.k_sum_range(L, f,0, smin - 1)
    ...
    def get_g_y_conj_value(self,g_x,y_val,x,constraint):
        objective = cp.Maximize(cp.sum(y_val* x)- g_x.sum())
    ...
    def p_f(self,x,L):     # 1*F的表达式
        ...
        for f in range(self.len_F):
            ...
            function_p_f=function_p_f/(self.k_star+1)
    ...
    def q_f(self,x,L):    # 1*F的表达式 ,x为1*M变量
        ...
        for f in range(self.len_F):
            ...
            function_q_f=function_q_f/(self.k_star)
...
a_r=np.random.uniform(size=(len_M),low=0,high=1)
b_r=-np.random.uniform(size=(len_M),low=0,high=1)
c_r=np.random.uniform(size=(len_M),low=1,high=3)
g=np.ones(shape=(len_M))
o=np.random.uniform(size=(len_M),low=0,high=1)
o=o/o.sum()
